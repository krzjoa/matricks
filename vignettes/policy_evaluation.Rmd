...
ggplot(data = x2, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile() +
  scale_x_continuous(breaks = 1:ncol(x) + 0.5, labels = 1:ncol(x)) +
...
ggplot(x2, aes(x = Var2, y = Var1)) + 
  geom_raster(aes(fill = value)) + 
  scale_x_continuous(breaks = 1:ncol(x), labels = 1:ncol(x)) +
  scale_y_continuous(breaks = 1:nrow(x), labels = 1:nrow(x)) +
  theme(
    panel.background = element_rect(fill = NA),
    panel.ontop = TRUE
) + 
...
plot_matrix <- function(x, ..., show.grid = ){
  x <- reshape2::melt(x)
  ggplot(x, aes(x = Var2, y = Var1)) + 
    geom_raster(aes(fill = value)) + theme(
  panel.background = element_rect(fill = NA),
  panel.ontop = TRUE
)
}
